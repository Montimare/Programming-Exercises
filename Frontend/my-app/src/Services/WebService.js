...
export const fetchUsers = async () => {
    return await axios.get("http://backend:8000/users/");
};

export const fetchEvents = async () => {
    return await axios.get("http://backend:8000/events/");
};

export const fetchEventsByUser = async (userID) => {
    return await axios.get("http://backend:8000/users/" + userID + "/events/");
};

export const fetchEventListsByUser = async (userID) => {
    return await axios.get("http://backend:8000/users/" + userID + "/eventlists/");
};

export const fetchNotificationsByUser = async (userID) => {
    return await axios.get("http://backend:8000/users/" + userID + "/notifications/");
};

export const fetchGroupsByUser = async (userID) => {
    return await axios.get("http://backend:8000/users/" + userID + "/groups/");
}
